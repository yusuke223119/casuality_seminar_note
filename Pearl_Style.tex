...
\begin{proof}
  ...
  命題3.8により，$nd(X_i) \cup \{X_i\}$ は祖先集合であるから，命題4.2より$G_{nd(X_i) \cup \{X_i\}}$ はベイジアンネットワークである.
...
\begin{proof}
  定理 4.2 より，$G_{An(\{X_i\} \cup \{X_j\} \cup S)}$ はベイジアンネットワークであるから，定理 3.14 により大域マルコフ性が証明される.
\end{proof}
...
\begin{proof}
  定理 4.18, 4.19 により，$S$ は無向グラフ $G^m_{An(S \cup \{X_i, X_j\})}$ を（そのときに限り）分離する.
  ゆえに，系 4.17 により，
  \begin{equation}
      X_i \perp X_j \mid S
  \end{equation}
...
\begin{defn}{因果ダイアグラム}
  $X_1, \dots, X_n$ を確率変数、$V = \{X_1, \dots, X_n\}$ を頂点集合とした DAG $G = (V, E)$ を考える。各 $X_i$ が関数 $f_i$ と誤差変数 $\varepsilon_i$ を用いて、
  \[
    X_i = f_i(pa(X_i), \varepsilon_i) 
  \]
  と記述されるとする。このとき、$G$ を因果ダイアグラムという。ただし、誤差変数 $\varepsilon_i$ は外生的、つまり、
  \begin{enumerate}
...
\[
  X_i = f_i(pa(X_i), \varepsilon_i) 
\]
であるため、$X_i \mid pa(X_i), \varepsilon_i$ は定数である。ゆえに、
\[ 
  X_i \perp U \mid pa(X_i), \varepsilon_i 
\]
が成立する。
