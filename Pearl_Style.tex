...
さて，この介入後の分布 $P(X_1, \dots, X_n \mid do(X_i = x_i))$ は定理~\ref{prop:SCM_is_BN}から $G$ におけるベイジアンネットワークであることが保証されているため，同時確率は次のように分解できる.
\begin{align*}
...
\begin{exm}{IPW推定}
  ...
  \begin{align}
    ...
    &= E\left[\frac{YI(X=x)}{P(X \mid Z)}\right]
  \end{align}
  分母の $P(X \mid Z)$ は傾向スコアと呼ばれ，$e(Z)$ で表す.今，同分布から独立に抽出された標本 $(x_1, y_1, z_1), \dots, (x_n, y_n, z_n)$ について，推定量
  \begin{equation}
...
\begin{proof}
  ...
  \noindent[規則2] $(Y \perp Z \mid X, W)_{G_{\bar{X}\underline{Z}}}$ から，
  \begin{equation}
  ...
  \noindent[規則3] $(Y \perp Z \mid X, W)_{G_{\bar{X}, \overline{Z(W)}}}$ が成り立つことを確認する．
  ...
  \noindent \textbf{(ii) $Y$ と $F_z$ のパス $p$ が $Z$ へのバックドアパスを含む場合:}
...
\begin{exm}{喫煙と肺がん}
  喫煙と肺がんの因果関係は医学界，統計学会で大論争を引きおこした.
  当時の医学データでは，喫煙者と肺がん患者には高い正の相関があることが知られていた.
  この問題について R.A.Fisher は，「相関は因果を意味しない」と言い，「たばこを好む遺伝子が肺がんを引きおこす」という未観測の交絡の可能性を指摘した．
  ...
  \begin{enumerate}
    ...
    \item $X \leftarrow U \to Y \leftarrow C$ がバックドアパスであるが， $Y$ が合流点となっておりブロックされている．
    \item $C \leftarrow X \leftarrow U \to Y$ がバックドアパスであるが， $\{X\}$ により，このパスはブロックされる．
  \end{enumerate}
