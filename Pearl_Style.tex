...
\begin{prop}{バックドア基準}
  ...
  $V$ の部分集合 $Z$ がすべてのバックドアパスをブロックするとき，$Z$ は\textbf{バックドア基準}を満たすという.このとき，
  \begin{equation}
...
\begin{proof}
  ...
  \begin{equation}
    X = f_X(pa(X))
  \end{equation}
  であるから，任意の $W \in V \cup de(X_k^*) \cup U$ について，
  \begin{equation}
    X \perp W \mid pa(X)
  \end{equation}
...
\begin{proof}
  ...
  一方，$Y$ が $X$ の子孫でないため $X$ から $Y$ への有向道が存在しない．
  よって， $X$ から出る矢線を含む $X$ と $Y$ のパスはすべて空集合でブロックされ， $Z$ は $X$ の子孫を含まず， $X$ から出る矢線を含む $X$ と $Y$ のパスは，必ず $Z$ に含まれない合流点をもつ.
  よって $X$ と $Y$ は $Z$ によって有向分離されて
  \[
    X \perp Y_x \mid Z
  \]
  が証明される.
...
ここまでで主要なRubin流の仮定をPearl流の考え方で整理することができた.
しかしポテンシャルアウトカムは流派の壁を越えて重要な概念である.
次の\textbf{因果の梯子}はJudea Pearl先生が提唱した因果推論の問題を整理する概念である.

\noindent \textbf{\underline{因果の梯子}}
